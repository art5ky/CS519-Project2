...
void *touch_pages(void *arg) {
    ...
    // Enable extents for just this thread thread
    if (syscall(SYS_ENABLE_EXTENT) != 0) {
        perror("Error calling sys_enable_extent in thread");
    }
...
int main(int argc, char *argv[]) {
    ...
    /* Removed the following so that we can track beyond main thread
